...
void checkLeaseStatus(MYSQL* db) {
    ...
    if (row == NULL) {
        ...
        if (strcmp(status, "Rejected") == 0) {
            ...
            // Collect review
            char rating[255];
            char additionalFeedback[255];

            printf("\nYour lease is fully complete. Please provide a review.\n");
            printf("Rating (e.g., Excellent, Good, Fair): ");
            scanf(" %[^\n]%*c", rating);
            printf("Additional Feedback: ");
            scanf(" %[^\n]%*c", additionalFeedback);

            // Insert review into the Review table
            sprintf(query,
                "INSERT INTO Review (StudentId, ReviewDate, Rating, AdditionalFeedback) "
                "VALUES (%d, CURDATE(), '%s', '%s');",
                studentId, rating, additionalFeedback);

            if (mysql_query(db, query) != 0) {
                printf("Error: Unable to save review. %s\n", mysql_error(db));
            }
            else {
